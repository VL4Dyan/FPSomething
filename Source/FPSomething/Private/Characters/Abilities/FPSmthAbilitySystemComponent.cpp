...
static TAutoConsoleVariable<float> CVarReplayMontageErrorThreshold(
	TEXT("FPSmth.replay.MontageErrorThreshold"),
	0.5f,
	TEXT("Tolerance level for when montage playback position correction occurs in replays")
);

void UFPSmthAbilitySystemComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(UFPSmthAbilitySystemComponent, RepAnimMontageInfoForMeshes);
}

UFPSmthAbilitySystemComponent* UFPSmthAbilitySystemComponent::GetAbilitySystemComponentFromActor(const AActor* Actor, bool LookForComponent)
{
	return Cast<UFPSmthAbilitySystemComponent>(UAbilitySystemGlobals::GetAbilitySystemComponentFromActor(Actor, LookForComponent));
}

bool UFPSmthAbilitySystemComponent::BatchRPCTryActivateAbility(FGameplayAbilitySpecHandle InAbilityHandle, bool EndAbilityImmediately)
{
	...
	if (InAbilityHandle.IsValid())
	{
		...
		if (EndAbilityImmediately)
		{
			...
			if (AbilitySpec)
			{
				UFPSmthGameplayAbility* Ability = Cast<UFPSmthGameplayAbility>(AbilitySpec->GetPrimaryInstance());
...
FGameplayAbilitySpecHandle UFPSmthAbilitySystemComponent::FindAbilitySpecHandleForClass(TSubclassOf<UGameplayAbility> AbilityClass, UObject* OptionalSourceObject)
{
...
void UFPSmthAbilitySystemComponent::CurrentMontageStopForMesh(USkeletalMeshComponent* InMesh, float OverrideBlendOutTime)
{
...
float UFPSmthAbilitySystemComponent::PlayMontageForMesh(UGameplayAbility* InAnimatingAbility, USkeletalMeshComponent* InMesh, FGameplayAbilityActivationInfo ActivationInfo, UAnimMontage* NewAnimMontage, float InPlayRate, FName StartSectionName, bool bReplicateMontage)
{
	UFPSmthGameplayAbility* InAbility = Cast<UFPSmthGameplayAbility>(InAnimatingAbility);
	...
	if (AnimInstance && NewAnimMontage)
	{
		...
		if (Duration > 0.f)
		{
			...
			else
			{
				...
				{
					PredictionKey.NewRejectedDelegate().BindUObject(this, &UFPSmthAbilitySystemComponent::OnPredictiveMontageRejectedForMesh, InMesh, NewAnimMontage);
				}
			}
		}
	}
...
UGameplayAbility* UFPSmthAbilitySystemComponent::GetAnimatingAbilityFromAnyMesh()
{
...
UAnimMontage* UFPSmthAbilitySystemComponent::GetCurrentMontageForMesh(USkeletalMeshComponent* InMesh)
{
	UAnimInstance* AnimInstance = IsValid(InMesh) && InMesh->GetOwner() == AbilityActorInfo->AvatarActor ? InMesh->GetAnimInstance() : nullptr;
	FGameplayAbilityLocalAnimMontageForMesh& AnimMontageInfo = GetLocalAnimMontageInfoForMesh(InMesh);
...
void UFPSmthAbilitySystemComponent::OnPredictiveMontageRejectedForMesh(USkeletalMeshComponent* InMesh, UAnimMontage* PredictiveMontage)
{
...
FGameplayAbilityLocalAnimMontageForMesh& UFPSmthAbilitySystemComponent::GetLocalAnimMontageInfoForMesh(USkeletalMeshComponent* InMesh)
{
...
FGameplayAbilityRepAnimMontageForMesh& UFPSmthAbilitySystemComponent::GetGameplayAbilityRepAnimMontageForMesh(USkeletalMeshComponent* InMesh)
{
...
void UFPSmthAbilitySystemComponent::AnimMontage_UpdateReplicatedDataForMesh(USkeletalMeshComponent* InMesh)
{
...
void UFPSmthAbilitySystemComponent::AnimMontage_UpdateReplicatedDataForMesh(FGameplayAbilityRepAnimMontageForMesh& OutRepAnimMontageInfo)
{
...
float UFPSmthAbilitySystemComponent::PlayMontageSimulatedForMesh(USkeletalMeshComponent* InMesh, UAnimMontage* NewAnimMontage, float InPlayRate, FName StartSectionName)
{
	float Duration = -1.f;
	UAnimInstance* AnimInstance = IsValid(InMesh) && InMesh->GetOwner() == AbilityActorInfo->AvatarActor ? InMesh->GetAnimInstance() : nullptr;
	if (AnimInstance && NewAnimMontage)
...
void UFPSmthAbilitySystemComponent::OnRep_ReplicatedAnimMontageForMesh()
{
...
bool UFPSmthAbilitySystemComponent::IsReadyForReplicatedMontageForMesh()
{
